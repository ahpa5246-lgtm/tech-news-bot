...
def load_verification_prompt() -> str:
    return (PROMPT_DIR / "source_verification_system.txt").read_text(encoding="utf-8")


def load_verification_schema() -> dict[str, Any]:
...
class OpenAICompatibleProvider:
    ...
    def _complete(self, system_prompt: str, payload: dict[str, Any], schema_name: str, schema: dict[str, Any]) -> dict[str, Any]:
        body = {
            "model": self.model,
            "messages": [
        ...
        request = Request(endpoint, data=json.dumps(body, ensure_ascii=False).encode("utf-8"), headers={"Authorization": f"Bearer {self.settings.api_key}", "Content-Type": "application/json", "User-Agent": "TechNewsCollector/3.0"}, method="POST")
        try:
    ...
    def analyze_article(self, article_payload: dict[str, Any]) -> dict[str, Any]:
        body = {
            ...
            "response_format": {"type": "json_schema", "json_schema": {"name": "news_editorial_analysis", "strict": True, "schema": load_schema()}},
